...
import * as tauriNative from "@/lib/tauri-native";
...
import { 
  ...
  Target, Check, Save, Download, FileCode
} from "lucide-react";
...
export default function AeThexOS() {
  ...
  useEffect(() => {
    const bootSequence = async () => {
      ...
      try {
        // Use auth context instead of API call for desktop compatibility
        if (user) {
          foundIdentity = true;
          setDetectedIdentity({ 
            username: user.email?.split('@')[0] || 'User', 
            passportId: user.id?.slice(0, 8).toUpperCase() 
          });
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        if (user) {
          const data = await dataService.fetchNotifications(user.id);
  ...
  // Listen for system tray quick action events
  ...
  useEffect(() => {
    ...
    const setupListener = async () => {
      try {
        ...
        unlisten = await tauriEvent.listen('open-app', (event: any) => {
          ...
          const appToOpen = appMap[appName] || appName;
          const existingApp = openApps.find((a: any) => a.name === appToOpen);
          
          if (!existingApp) {
            handleLaunchApp(appToOpen);
          } else {
            setActiveApp(appToOpen);
          }
        });
      } catch (err) {
        // Not on desktop, ignore
        console.debug('[OS] Tray listener not available:', err);
      }
    };
  ...
  }, [handleLaunchApp, openApps, setActiveApp]);
  */
  ...
  // Motion values for mobile gestures and desktop parallax (framer-motion hooks)
  const dragX = useMotionValue(0);
  const dragOpacity = useTransform(dragX, [-200, 0, 200], [0.5, 1, 0.5]);
  
  // Parallax calculations for desktop background
  ...
  // ========== END OF HOOKS - Conditional returns can happen after this point ==========
...
function DesktopWidgets({ time, weather, notifications }: { 
  ...
  const { data: metrics } = useQuery({
    queryKey: ['os-metrics'],
    queryFn: () => dataService.fetchMetrics(),
              ...
              >
                ...
                <div className="p-4 space-y-4">
                  ...
                  <button
                    onClick={async () => {
                      try {
                        await dataService.trackEvent('upgrade-click', {
                          source: 'tray-upgrade',
    ...
    switch (baseCmd) {
      ...
      case 'status':
        try {
          ...
          await delay(300);
          const data = await dataService.fetchMetrics();
          await typeEffect([
            "┌─────────────────────────────────┐",
            "│       SYSTEM STATUS             │",
            "├─────────────────────────────────┤",
      ...
      case 'architects':
        try {
          ...
          await delay(400);
          const data = await dataService.fetchAllProfiles();
          if (!data.length) {
...
function PassportApp({ onLoginSuccess, isDesktopLocked }: { onLoginSuccess?: () => void; isDesktopLocked?: boolean }) {
  ...
  const { data: userProfile } = useQuery({
    ...
    queryFn: async () => {
      ...
      return dataService.fetchUserProfile(user.id);
    },
...
function FilesApp() {
  const { data: projects, isLoading } = useQuery({
    ...
    queryFn: () => dataService.fetchProjects(),
  });
  ...
  const handleExportProject = async (project: any) => {
    try {
      await tauriNative.saveProject(project);
      await tauriNative.showNotification(
        'Project Exported',
  ...
  const handleSelectFolder = async () => {
    try {
      const folder = await tauriNative.selectFolder();
      if (folder) {
        await tauriNative.showNotification('Folder Selected', folder);
      }
    } catch (err) {
    ...
    <div className="min-h-full bg-slate-950 flex flex-col">
      <div className="flex items-center gap-2 p-2 bg-slate-900 border-b border-white/10">
        ...
        <button
          ...
          className="px-2 md:px-3 py-1 bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-400 text-xs rounded flex items-center gap-1 transition-colors shrink-0"
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            ...
            {projects?.length > 0 ? (
              <div className="space-y-2">
                {projects.map((p: any) => (
                  <div key={p.id} className="flex items-center gap-3 p-3 bg-white/5 rounded border border-white/10 hover:border-cyan-500/30 transition-colors group">
                    ...
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 p-1.5 hover:bg-cyan-500/20 text-cyan-400 rounded transition-all"
...
function AchievementsApp() {
  ...
  const { data: userAchievements = [], isLoading: achievementsLoading } = useQuery({
    ...
    queryFn: () => user?.id ? dataService.fetchUserAchievements(user.id) : Promise.resolve([]),
    enabled: !!user,
  });

  const { data: allAchievements = [], isLoading: allLoading } = useQuery({
    queryKey: ['all-achievements'],
    queryFn: () => dataService.fetchAllAchievements(),
    enabled: !!user,
  });
  ...
  const unlockedIds = new Set((Array.isArray(userAchievements) ? userAchievements : []).map((a: any) => a.achievement_id || a.id));
  
  // Combine unlocked and locked achievements
  const achievements = [
    ...(Array.isArray(userAchievements) ? userAchievements : []).map((a: any) => ({ ...a, unlocked: true })),
    ...(Array.isArray(allAchievements) ? allAchievements : []).filter((a: any) => !unlockedIds.has(a.id)).map((a: any) => ({ ...a, unlocked: false }))
  ];
    ...
    <div className="min-h-full bg-slate-950 p-3 md:p-4 overflow-auto">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="ml-auto text-xs text-white/40 font-mono shrink-0">
          {(Array.isArray(userAchievements) ? userAchievements : []).length} / {(Array.isArray(allAchievements) ? allAchievements : []).length}
        </span>
      </div>
...
function OpportunitiesApp() {
  const { data: opportunities, isLoading } = useQuery<any[]>({
    ...
    queryFn: () => dataService.fetchOpportunities(),
  });
...
function EventsApp() {
  const { data: events, isLoading } = useQuery<any[]>({
    queryKey: ['/api/events'],
    queryFn: () => dataService.fetchEvents(),
  });
...
function NetworkMapApp() {
  const { data: architects } = useQuery({
    ...
    queryFn: () => dataService.fetchAllProfiles(),
  });
...
class MetaverseRegistry {
  ...
  const handleSaveFile = async () => {
    try {
      await tauriNative.saveFile(fileName, code);
    } catch (err) {
  ...
  const handleOpenFile = async () => {
    try {
      const result = await tauriNative.openFile();
      if (result) {
        setCode(result.content);
        // Extract filename from path
        const pathParts = result.path.split(/[\\/]/);
        setFileName(pathParts[pathParts.length - 1] || 'untitled.ts');
      }
    } catch (err) {
    ...
    <div className="min-h-full bg-[#1e1e1e] flex flex-col">
      <div className="flex items-center gap-2 px-2 md:px-4 py-2 bg-[#252526] border-b border-[#3c3c3c] overflow-x-auto">
        <div className="flex items-center gap-2 px-2 md:px-3 py-1 bg-[#1e1e1e] rounded-t border-t-2 border-cyan-500 shrink-0">
          ...
          <span className="text-xs md:text-sm text-white/80">{fileName}</span>
          <span className="text-white/30 text-xs">~</span>
        </div>
        <div className="ml-auto flex items-center gap-1 md:gap-2 shrink-0">
          <button
            onClick={handleOpenFile}
            className="text-[10px] md:text-xs text-white/50 hover:text-white px-1.5 md:px-2 py-1 bg-white/5 hover:bg-white/10 rounded flex items-center gap-1 transition-colors"
            title="Open File"
          >
            <FolderOpen className="w-3 h-3" />
...
function NewsFeedApp() {
  const { data: activities, isLoading, refetch } = useQuery({
    ...
    queryFn: () => dataService.fetchActivities(20),
  });
...
function NetworkNeighborhoodApp({ openIframeWindow }: { openIframeWindow?: (url: string, title: string) => void }) {
  const { data: founders = [], isLoading } = useQuery({
    ...
    queryFn: () => dataService.fetchLeaderboard(),
  });

  useEffect(() => {
    if (!isLoading) {
      dataService.trackEvent('directory_view', {
        source: 'networkneighborhood',
...
function FoundryApp({ openIframeWindow }: { openIframeWindow?: (url: string, title: string) => void }) {
  ...
  useEffect(() => {
    dataService.trackEvent('foundry_open', {
      source: 'foundry-app',
      timestamp: new Date().toISOString()
    });
  }, []);
          ...
          {files.map((file, idx) => (
            <motion.button
              ...
              onClick={() => {
                ...
                try {
                  dataService.trackEvent('intel_open', {
                    source: 'intel-app',
          ...
          {drives.map((drive) => (
            <motion.div
              ...
              onClick={() => {
                ...
                if (drive.id === 'D') {
                  dataService.trackEvent('drive_d_open', {
                    source: 'drives-app',
                    timestamp: new Date().toISOString()
                  });
                }
